...
unsigned long Current_color = RGB_white;
...
static void Set_Color(unsigned long color_data)
{
	unsigned char j;
...
unsigned long Luminance_proportional_control(unsigned long color_num,float proportional)
{
	unsigned char Red, Green, Blue;  //三原色
	// 红 绿 蓝 三原色分解
	Red = color_num>>16;
	Green   = color_num>>8;	
	Blue  = color_num;
	...
	// 计算出各色比例值
	Red   = (float)Red*proportional;
	Green = (float)Green*proportional;
	Blue  = (float)Blue*proportional;
	
	color_num  = Red<<16 | Green<<8 | Blue;  // 合成  红绿蓝
